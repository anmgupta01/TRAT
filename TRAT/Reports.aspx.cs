    ...
    public partial class Reports : System.Web.UI.Page
    {
        ...
        protected void btnDownloadReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet ds = new DataSet();
                string message = "";
                ds = dh.GetFilteredEmployeeReport(reportType, dtStartDate.Value.ToString(), dtEndDate.Value.ToString(), int.Parse(drpServiceLine.SelectedValue),
        ...
        private void ExportDataSetToExcel(DataSet ds)
        {
            try
            {
                ...
                Excel.Workbook excelWorkBook = excelApp.Workbooks.Open(@"C:\My Data\TRAT - DATa\EmployeeData.xlsx");

                //foreach (DataTable table in ds.Tables)
                //{
                //Add a new worksheet to workbook with the Datatable name
                Excel.Worksheet excelWorkSheet = excelWorkBook.Sheets.Add();
                //excelWorkSheet.Name = table;

                for (int i = 1; i < ds.Tables[0].Columns.Count + 1; i++)
                {
                    excelWorkSheet.Cells[1, i] = ds.Tables[0].Columns[i - 1].ColumnName;
                }
                ...
                for (int j = 0; j < ds.Tables[0].Rows.Count; j++)
                {
                    ...
                    {
                        excelWorkSheet.Cells[j + 2, k + 1] = ds.Tables[0].Rows[j].ItemArray[k].ToString();
                    }
                }
                //}
        ...
        protected void ExportExcel(DataSet ds)
        {
            try
            {
                ...
                Excel.Application excelApp = new Excel.Application();
                //Excel.Workbook excelWorkBook  =new  excelApp.Workbook() ;
